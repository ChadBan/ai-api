...
func (s *ChannelService) GetLatestModels(channelType, apiKey, baseURL string) ([]string, error) {
	// 尝试从官方API获取最新模型列表
	models, err := s.fetchModelsFromAPI(channelType, apiKey, baseURL)
	if err == nil && len(models) > 0 {
		return models, nil
	}

	// 如果从API获取失败，使用预定义的模型列表作为备用
	return s.getPredefinedModels(channelType), nil
}

// fetchModelsFromAPI 从官方API获取模型列表
func (s *ChannelService) fetchModelsFromAPI(channelType, apiKey, baseURL string) ([]string, error) {
	switch channelType {
	case "1": // OpenAI
		return s.fetchOpenAIModels(apiKey, baseURL)
	case "2": // Anthropic
		return s.fetchAnthropicModels(apiKey, baseURL)
	case "4": // Google Gemini
		return s.fetchGeminiModels(apiKey, baseURL)
	case "14": // 豆包
		return s.fetchDoubaoModels(apiKey, baseURL)
	default:
		// 其他渠道暂时使用预定义模型
		return nil, errors.New("API not implemented for this channel type")
	}
}

// fetchOpenAIModels 从OpenAI API获取模型列表
func (s *ChannelService) fetchOpenAIModels(apiKey, baseURL string) ([]string, error) {
	// 实际项目中，这里应该从OpenAI API获取模型列表
	// 如果提供了API Key和BaseURL，则调用真实API
	if apiKey != "" && baseURL != "" {
		// 构建API请求
...
// fetchAnthropicModels 从Anthropic API获取模型列表
func (s *ChannelService) fetchAnthropicModels(apiKey, baseURL string) ([]string, error) {
	// 实际项目中，这里应该从Anthropic API获取模型列表
	// 如果提供了API Key和BaseURL，则调用真实API
	if apiKey != "" && baseURL != "" {
		// 构建API请求
		req, err := http.NewRequest("GET", baseURL+"/models", nil)
...
// fetchGeminiModels 从Google Gemini API获取模型列表
func (s *ChannelService) fetchGeminiModels(apiKey, baseURL string) ([]string, error) {
	// 实际项目中，这里应该从Google Gemini API获取模型列表
	// 如果提供了API Key和BaseURL，则调用真实API
	if apiKey != "" && baseURL != "" {
		// 构建API请求
		req, err := http.NewRequest("GET", baseURL+"/models", nil)
		...
		for _, model := range result.Models {
			...
			if len(parts) > 0 {
				models = append(models, parts[len(parts)-1])
			}
		}
...
// fetchDoubaoModels 从豆包API获取模型列表
func (s *ChannelService) fetchDoubaoModels(apiKey, baseURL string) ([]string, error) {
	// 实际项目中，这里应该从豆包API获取模型列表
	// 如果提供了API Key和BaseURL，则调用真实API
	if apiKey != "" && baseURL != "" {
		// 构建API请求
		req, err := http.NewRequest("GET", baseURL+"/models", nil)
...
// getPredefinedModels 获取预定义的模型列表
func (s *ChannelService) getPredefinedModels(channelType string) []string {
