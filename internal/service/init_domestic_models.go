...
func InitDomesticModels(db *gorm.DB, logger *zap.Logger) {
	logger.Info("Initializing domestic LLM models...")
	...
	for _, cfg := range domesticChannels {
		...
		if count == 0 {
			...
			if err := db.Create(&channel).Error; err != nil {
				logger.Warn("Failed to create domestic channel", zap.String("name", cfg.name), zap.Error(err))
			} else {
				logger.Info("Created domestic channel", zap.String("name", cfg.name), zap.Int("type", cfg.channelType))
			}
		}
	}
	...
	for _, m := range domesticModels {
		...
		if count == 0 {
			...
			if err := db.Create(&modelItem).Error; err != nil {
				logger.Warn("Failed to create domestic model", zap.String("name", m.name), zap.Error(err))
			} else {
				logger.Info("Created domestic model", zap.String("name", m.name), zap.String("displayName", m.displayName))
			}
		}
	}

	logger.Info("Domestic LLM models initialization completed")
}
