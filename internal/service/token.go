...
func NewTokenService(db *gorm.DB, logger *zap.Logger, billingService *BillingService) *TokenService {
	return &TokenService{
		db:             db,
		logger:         logger,
		billingService: billingService,
...
func (s *TokenService) CheckModelPermission(token *model.Token, modelName string) error {
	...
	if err := json.Unmarshal([]byte(token.ModelLimit), &modelLimit); err != nil {
		s.logger.Warn("failed to parse model_limit", zap.String("model_limit", token.ModelLimit), zap.Error(err))
	...
	if balance.Quota >= renewalCost*1000000 {
		...
		return s.db.Transaction(func(tx *gorm.DB) error {
			...
			s.logger.Info("Token auto-renewed successfully",
				...
				zap.Int("renewal_quota", config.RenewalQuota),
				zap.Int("user_balance_deducted", renewalCost*1000000))
