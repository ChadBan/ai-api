...
func (s *AuditService) Log(ctx context.Context, opts LogOptions) error {
	...
	s.logger.Info("audit_log",
		...
		zap.String("resource", string(opts.Resource)),
		zap.String("resource_id", opts.ResourceID),
		zap.String("ip", opts.IPAddress),
		zap.Int("status", opts.StatusCode),
...
func NewErrorLogService(db *gorm.DB, logger *zap.Logger) *ErrorLogService {
...
func (s *ErrorLogService) LogError(level, message, file string, line int, function string, stackTrace string, context map[string]interface{}, userID int64, requestID string) error {
	...
	switch level {
	case "critical":
		s.logger.Fatal(message, zap.String("request_id", requestID))
	case "error":
		s.logger.Error(message, zap.String("request_id", requestID))
	default:
		s.logger.Warn(message, zap.String("request_id", requestID))
	}
