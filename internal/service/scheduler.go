...
func (s *SchedulerService) RegisterTask(name, schedule string, handler func() error) {
	...
	s.logger.Info("task registered", zap.String("name", name), zap.String("schedule", schedule))
}
...
func (s *SchedulerService) runTask(task *ScheduledTask) {
	...
	if err != nil {
		s.logger.Error("failed to parse schedule", zap.String("task", task.Name), zap.Error(err))
		return
	}

	s.logger.Info("task started", zap.String("name", task.Name), zap.Duration("interval", interval))
...
func (s *SchedulerService) executeTask(task *ScheduledTask) {
	s.logger.Info("executing task", zap.String("name", task.Name))
...
func (s *SchedulerService) testChannels() error {
	...
	for _, ch := range channels {
		...
		if err != nil {
			failCount++
			s.logger.Debug("channel test failed", zap.String("name", ch.Name), zap.Error(err))
			
			// 连续失败多次则禁用渠道
			if ch.ResponseTime > 1000 { // 简单判断：响应时间过长
				s.db.Model(&model.Channel{}).Where("id = ?", ch.ID).Update("status", 0)
				s.logger.Warn("disabled channel due to poor performance", zap.String("name", ch.Name))
			}
		} else {
			successCount++
			s.logger.Debug("channel tested", zap.String("name", ch.Name), zap.Bool("success", true))
		}
	}
...
func (s *SchedulerService) dailySummary() error {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&userID, &totalRequests, &inputTokens, &outputTokens); err != nil {
			s.logger.Error("failed to scan daily summary", zap.Error(err))
			continue
		}
	...
	s.logger.Info("daily summary completed", zap.String("date", yesterdayStr))
	return nil
}
	...
	for _, token := range expiredTokens {
		if err := s.db.Model(&token).Update("status", 0).Error; err != nil {
			...
			s.logger.Info("disabled expired token",
				zap.Int64("token_id", token.ID),
				zap.String("expired_time", expiredTime),
			)
		}
	}

	s.logger.Info("token cleanup completed", zap.Int("disabled_count", len(expiredTokens)))
	return nil
}
...
func (s *SchedulerService) autoReconciliation() error {
	...
	s.logger.Info("starting auto reconciliation", zap.String("date", yesterdayStr))
	...
	if err != nil {
		s.logger.Error("reconciliation query failed", zap.Error(err))
		return err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		s.logger.Warn("reconciliation discrepancy found",
			...
			zap.Int64("usage_count", m.UsageCount),
			zap.Int64("billing_count", m.BillingCount),
			zap.Int64("usage_tokens", m.UsageTokens),
			zap.Int64("billing_quota", m.BillingQuota),
		)
	}
	...
	s.logger.Info("reconciliation report generated",
		...
		zap.Int("total_discrepancies", discrepancyCount),
	)
	...
	if discrepancyCount > 0 {
		s.logger.Warn("reconciliation completed with discrepancies",
			zap.Int("count", discrepancyCount),
		)
	} else {
...
func (s *SchedulerService) cleanupOldLogs() error {
	...
	s.logger.Info("cleaned old audit logs", zap.Int64("deleted", result.RowsAffected))

	// 清理请求日志
	result = s.db.Where("created_at < ?", cutoffTime).Delete(&model.RequestLog{})
	s.logger.Info("cleaned old request logs", zap.Int64("deleted", result.RowsAffected))
	...
	s.logger.Info("cleaned old error logs", zap.Int64("deleted", result.RowsAffected))
